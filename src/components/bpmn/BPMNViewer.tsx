...
const BPMNViewer: React.FC<BPMNViewerProps> = ({
  ...
  const calculateElementProgress = (elementId: string): { progress: number; color: string; showProgress: boolean; assignee: User | null } => {
    ...
    linkedIssues.forEach(issue => {
      ...
      } else if (issue.status === 'IN_PROGRESS' && issue.progress !== undefined) {
    ...
    if (progressValues.length === 0) {
      return { progress: 0, color: '#e0e0e0', showProgress: false, assignee: null };
    }
    
    const averageProgress = Math.round(progressValues.reduce((sum, val) => sum + val, 0) / progressValues.length);
  ...
  const createAvatarAndProgressBar = (element: any, overlays: any) => {
    ...
    const { progress, color, showProgress, assignee } = calculateElementProgress(elementId);
    
    if (!showProgress) {
      return;
    }
    
    const avatarSrc = assignee?.avatar || 'https://mui.com/static/images/avatar/1.jpg';
